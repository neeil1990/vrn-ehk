...
class RestService extends \IRestService
{
	...
	protected static function checkParamsBeforeSettingsUpdate($params)
	{
		if (!isset($params['ID']))
		{
			throw new RestException('Parameter ID is not defined', self::ERROR_VALIDATION_FAILURE);
		}

		$item = Manager::getById($params['ID']);
		if (!$item)
		{
			throw new RestException('Pay system not found', static::ERROR_VALIDATION_FAILURE);
		}
	...
	protected static function checkParamsBeforeSettingsGet($params)
	{
		...
		{
			throw new RestException('Parameter PERSON_TYPE_ID is not defined', self::ERROR_VALIDATION_FAILURE);
		}
	...
	public static function addHandler(array $params)
	{
		...
		self::validateParamsOnAddHandler($params);
	 ...
	 * @throws \Bitrix\Main\SystemException
	 */
	private static function validateParamsOnAddHandler(array $params)
	{
		if (!isset($params['NAME']))
		{
			throw new RestException('Parameter NAME is not defined', self::ERROR_VALIDATION_FAILURE);
		}

		if (!isset($params['CODE']))
		{
			throw new RestException('Parameter CODE is not defined', self::ERROR_VALIDATION_FAILURE);
		}

		if (!isset($params['SETTINGS']))
		{
			throw new RestException('Parameter SETTINGS is not defined', self::ERROR_VALIDATION_FAILURE);
		}

		if (!isset($params['SETTINGS']['CODES']))
		{
			throw new RestException('Parameter SETTINGS[CODES] is not defined', self::ERROR_VALIDATION_FAILURE);
		}
	...
	public static function getSettingsByInvoice(array $params)
	{
		...
		self::validateParamsForInvoice($params);
	...
	public static function getSettingsByPayment(array $params)
	{
		...
		self::validateParamsForPayment($params);
	 ...
	 * @throws RestException
	 */
	private static function validateParamsForInvoice(array $params)
	{
		if (!isset($params['BX_REST_HANDLER']) && !isset($params['PAY_SYSTEM_ID']))
		{
			throw new RestException('Empty field BX_REST_HANDLER and PAY_SYSTEM_ID', self::ERROR_VALIDATION_FAILURE);
		}
		...
		if (isset($params['PAY_SYSTEM_ID']))
		{
			...
			if (!$data)
			{
				throw new RestException('Pay system with ID='.$params['PAY_SYSTEM_ID'].' not found', static::ERROR_VALIDATION_FAILURE);
			}
		}
		...
		if (isset($params['BX_REST_HANDLER']))
		{
			...
			if (!$dbRes->fetch())
			{
				throw new RestException('Incorrect rest handler code', static::ERROR_VALIDATION_FAILURE);
			}
		}
	 ...
	 * @throws \Bitrix\Main\ArgumentException
	 */
	private static function validateParamsForPayment(array $params)
	{
		if (empty($params['PAY_SYSTEM_ID']))
		{
			throw new RestException('Empty field PAY_SYSTEM_ID!', self::ERROR_VALIDATION_FAILURE);
		}
		...
		{
			throw new RestException('Empty field PAYMENT_ID', self::ERROR_VALIDATION_FAILURE);
		}
		...
		if (!$dbRes->fetch())
		{
			throw new RestException(
				...
				' not found', self::ERROR_VALIDATION_FAILURE
			);
		}
	}
